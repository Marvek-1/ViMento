...
def _exchange_credentials(exchange_id: str) -> dict[str, str]:
    """Never attach credentials to the public market-data client.

    OHLCV and ticker endpoints are public. Keeping this boundary
    unconditional prevents a shared environment file from silently turning
    a data process into an authenticated exchange client. Live execution
    connectors own their credentials separately.
    """
    del exchange_id
    return {}
