...
class BaseEngine(ABC):
    ...
    def run_backtest(
        ...
        if bench_ticker and bench_ticker != "auto":
            ...
            benchmark_metadata = {
                "benchmark_status": bench_result.receipt.status,
                "benchmark_market": bench_result.receipt.market,
                "benchmark_ticker": bench_result.receipt.ticker,
                "benchmark_bars": bench_result.receipt.bars,
                "benchmark_requested_source": bench_result.receipt.requested_source,
                "benchmark_fetch_source": bench_result.receipt.fetch_source,
                "benchmark_error": bench_result.receipt.error,
                "benchmark_coverage_ratio": bench_result.receipt.coverage_ratio,
                "benchmark_coverage_ok": bench_result.receipt.coverage_ok,
                "benchmark_window_integrity_error": bench_result.receipt.window_integrity_error,
            }
            if bench_result.ok:
                # Only a resolved, fetched, valid benchmark may replace the
                # in-universe comparison series and feed excess/information
                # ratio -- a failed fetch must not silently fall back to a
                # different (and differently-labeled) comparison.
                bench_ret = bench_result.ret_series.reindex(dates).fillna(0.0)
                benchmark_metadata["benchmark_return"] = bench_result.total_ret
            else:
                # Leave bench_ret as the in-universe fallback for the equity
                # chart artifact, but the requested external comparison is
                # unavailable -- null out every metric that would otherwise
                # be silently computed against that fallback series and
                # mislabeled as a comparison to the failed ticker.
                benchmark_metadata["benchmark_return"] = None
                benchmark_metadata["excess_return"] = None
                benchmark_metadata["information_ratio"] = None
        # ── External benchmark fetch ──────────────────────────────────────────
