...
def _write_run_card(run_dir: Path, metrics: dict | None) -> None:
    ...
    (run_dir / "config.json").write_text('{"codes": ["000300.SH"]}\n', encoding="utf-8")
...
class SignalEngine:
    def generate(self, data_map):
        return {code: frame["close"] * 0 + 1 for code, frame in data_map.items()}
""".lstrip(),
            encoding="utf-8",
        )
    (artifacts_dir / "metrics.csv").write_text("sharpe\n1.0\n", encoding="utf-8")
    (artifacts_dir / "equity.csv").write_text("date,equity\n", encoding="utf-8")
    ...
    card: dict = {
        ...
        "run_purpose": "hypothesis_test",
        "reproducibility": {
            "strategy_hash": hashlib.sha256(signal_path.read_bytes()).hexdigest()
        },
    }
...
def test_link_rejects_missing_required_evidence_artifacts(
    isolated_env: Path,
) -> None:
    hyp_id = _make_hypothesis()
    run_dir = _run_dir(isolated_env)
    ...
    (run_dir / "run_card.json").write_text(
        json.dumps({"schema_version": 1, "metrics": {"sharpe": 1.0}}),
        encoding="utf-8",
    )
    ...
    assert result["status"] == "error"
    assert "missing required hypothesis evidence artifacts" in result["error"]
    assert "artifacts/metrics.csv" in result["error"]
    assert "artifacts/equity.csv" in result["error"]


def test_link_rejects_missing_run_card_evidence_metadata(
    isolated_env: Path,
) -> None:
    hyp_id = _make_hypothesis()
    run_dir = _run_dir(isolated_env)
    _write_run_card(run_dir, {"sharpe": 1.0})
    card_path = run_dir / "run_card.json"
    card = json.loads(card_path.read_text(encoding="utf-8"))
    card.pop("strategy_implementation_status")
    card.pop("provenance_valid")
    card.pop("run_purpose")
    ...
    assert result["status"] == "error"
    assert "not marked as hypothesis-valid implemented evidence" in result["error"]
    assert result["strategy_implementation_status"] == "missing"
    assert result["provenance_valid"] is False
    assert result["run_purpose"] == "missing"
    assert _get_hypothesis(hyp_id).run_cards == []


def test_link_rejects_bad_run_card_evidence_metadata(
    isolated_env: Path,
) -> None:
    hyp_id = _make_hypothesis()
    run_dir = _run_dir(isolated_env)
    ...
    card_path = run_dir / "run_card.json"
    card = json.loads(card_path.read_text(encoding="utf-8"))
    card["strategy_implementation_status"] = "implemented"
    card["provenance_valid"] = False
    card["run_purpose"] = "hypothesis_test"
    card_path.write_text(json.dumps(card), encoding="utf-8")
    ...
    assert result["status"] == "error"
    assert "not marked as hypothesis-valid implemented evidence" in result["error"]
    assert result["strategy_implementation_status"] == "implemented"
    assert result["provenance_valid"] is False
    assert result["run_purpose"] == "hypothesis_test"
    assert _get_hypothesis(hyp_id).run_cards == []
...
def test_link_rejects_header_only_trades_when_strategy_hash_matches_scaffold(
    isolated_env: Path,
) -> None:
    hyp_id = _make_hypothesis()
    run_dir = _run_dir(isolated_env)
    scaffold = json.loads(
        ScaffoldSignalEngineTool().execute(hypothesis_id=hyp_id, run_dir=str(run_dir))
    )
    assert scaffold["status"] == "ok"
    _write_run_card(run_dir, {"sharpe": 0.0, "total_return": 0.0})
    ...
    assert result["status"] == "error"
    assert "flat 0.0 placeholder" in result["error"]
    assert result["strategy_implementation_status"] == "scaffold"
    assert result["provenance_valid"] is False
    assert result["run_purpose"] == "smoke_only"
    reloaded = _get_hypothesis(hyp_id)
    assert reloaded.run_cards == []


def test_link_allows_implemented_strategy_with_zero_initialization(
    isolated_env: Path,
) -> None:
    hyp_id = _make_hypothesis()
    run_dir = _run_dir(isolated_env)
...
class SignalEngine:
    def generate(self, data_map: dict[str, "pd.DataFrame"]) -> dict[str, "pd.Series"]:
        ...
        for code, frame in data_map.items():
            ...
            signal.loc[close > close.rolling(20, min_periods=20).mean()] = 1.0
            signals[code] = signal
        return signals
""".lstrip(),
        encoding="utf-8",
    )
    _write_run_card(run_dir, {"sharpe": 0.5, "total_return": 0.02})
...
def test_link_allows_implemented_zero_trade_run(isolated_env: Path) -> None:
    hyp_id = _make_hypothesis()
    run_dir = _run_dir(isolated_env)
    code_dir = run_dir / "code"
    artifacts_dir = run_dir / "artifacts"
...
class SignalEngine:
    def generate(self, data_map):
        return {code: frame["close"] * 0 for code, frame in data_map.items()}
""".lstrip(),
        encoding="utf-8",
    )
    (artifacts_dir / "metrics.csv").write_text("trade_count,sharpe\n0,0\n", encoding="utf-8")
    (artifacts_dir / "equity.csv").write_text("date,equity\n", encoding="utf-8")
    (artifacts_dir / "trades.csv").write_text(
        "timestamp,code,side,price,qty,reason,pnl,holding_days,return_pct\n",
        encoding="utf-8",
    ...
    (run_dir / "run_card.json").write_text(
        json.dumps(
            {
                "schema_version": 1,
                "metrics": {"trade_count": 0, "sharpe": 0.0},
                "strategy_implementation_status": "implemented",
                "provenance_valid": True,
                "run_purpose": "hypothesis_test",
                "reproducibility": {
