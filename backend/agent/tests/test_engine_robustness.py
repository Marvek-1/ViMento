...
class TestSymbolIsolation:
    ...
    def test_backtest_records_explicit_benchmark_metadata(
        ...
        from backtest.benchmark import BenchmarkReceipt, BenchmarkResult
        ...
        def fake_resolve_benchmark(**kwargs):
            return BenchmarkResult(
                receipt=BenchmarkReceipt(
                    status="ok",
                    market="a_share",
                    requested_source=kwargs.get("source", "tushare"),
                    fetch_source="yfinance",
                    ticker="000300.SH",
                    explicit_ticker=kwargs.get("explicit"),
                    strategy_codes=list(kwargs.get("strategy_codes", [])),
    ...
    def test_backtest_records_failed_benchmark_as_receipt_not_crash(
        self,
        monkeypatch: pytest.MonkeyPatch,
        tmp_path: Path,
    ) -> None:
        """A failed external benchmark must not crash the run or fabricate evidence."""
        dates = pd.bdate_range("2024-04-01", periods=3)
        bars = pd.DataFrame(
            {
                "open": [10.0, 11.0, 12.0],
        ...
        def fake_resolve_benchmark(**kwargs):
            ...
            return BenchmarkResult(receipt=receipt, ret_series=pd.Series(dtype=float), total_ret=0.0)
        ...
        # Failure must be recorded, not silenced and not crashed past.
        assert metrics["benchmark_status"] == "fetch_failed"
        assert metrics["benchmark_error"] == "RuntimeError: fetch exploded"
        assert metrics["benchmark_ticker"] == "000300.SH"
        # A failed benchmark is not evidence: these must be null, not a
        # number computed against some other, mislabeled series.
        ...
        run_card_path = tmp_path / "run_card.json"
        run_card = json.loads(run_card_path.read_text(encoding="utf-8"))
        assert run_card["metrics"]["benchmark_status"] == "fetch_failed"
        assert run_card["metrics"]["benchmark_return"] is None
