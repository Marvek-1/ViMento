"""AgentLoop stream retry on ProviderStreamError.

Mirrors the swarm worker policy: a transient mid-stream failure (connection
reset, relay hiccup, 5xx) is retried with exponential backoff up to
STREAM_MAX_RETRIES times; a deterministic 4xx fails the run immediately with
...
def test_transient_stream_failure_is_retried_and_run_succeeds(
    ...
    assert reset["max_attempts"] == 3


def test_repeated_transient_failures_retried_up_to_max_and_run_succeeds(
    monkeypatch, tmp_path: Path
) -> None:
    """Three transient failures (within STREAM_MAX_RETRIES) then success → run completes."""
    llm = _FlakyLoopLLM(
    ...
    assert [reset["attempt"] for reset in resets] == [1, 2, 3]


def test_exhausted_retries_fails_run(monkeypatch, tmp_path: Path) -> None:
    """More transient failures than STREAM_MAX_RETRIES → failed run, no extra attempt."""
    llm = _FlakyLoopLLM(
        [_transient_error(), _transient_error(), _transient_error(), _transient_error()],
        "Final answer.",
    )
