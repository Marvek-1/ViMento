"""Tests for the stream retry with backoff on ProviderStreamError in run_worker.
...
with exponential backoff up to ``_STREAM_MAX_RETRIES`` times for
``ProviderStreamError`` — and only for it — before the existing failure path.
"""
...
def _run(monkeypatch, tmp_path: Path, llm: _FlakyChatLLM) -> WorkerResult:
    ...
    monkeypatch.setattr(worker_mod, "_STREAM_MAX_RETRIES", 3)
...
def test_repeated_stream_failures_retried_up_to_max_and_worker_succeeds(monkeypatch, tmp_path):
    """Three ProviderStreamErrors (within max) then success → worker completes (4 calls)."""
    llm = _FlakyChatLLM(
...
def test_exhausted_stream_failures_fails_worker(monkeypatch, tmp_path):
    """More ProviderStreamErrors than max retries → existing failure path (no extra try)."""
    llm = _FlakyChatLLM(
        [_stream_error(), _stream_error(), _stream_error(), _stream_error()],
        LLMResponse(content=FINAL_TEXT),
    )
