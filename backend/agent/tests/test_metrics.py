...
class TestBarsPerYear:
    def test_daily_auto(self) -> None:
        assert calc_bars_per_year("1D", "auto") == 252

    def test_daily_okx(self) -> None:
        assert calc_bars_per_year("1D", "okx") == 365

    def test_minute_auto(self) -> None:
        # 252 trading days × 240 minutes/day = 60480
        assert calc_bars_per_year("1m", "auto") == 252 * 240

    def test_hourly_okx(self) -> None:
        # 365 days × 24 hours/day = 8760
        assert calc_bars_per_year("1H", "okx") == 365 * 24

    @pytest.mark.parametrize("source", ["binance", "bybit", "gate", "ccxt"])
    def test_hourly_ccxt_exchange_sources(self, source: str) -> None:
        assert calc_bars_per_year("1H", source) == 365 * 24
    ...
    def test_unknown_interval(self) -> None:
        ...
        assert calc_bars_per_year("2H", "auto") == 252
