...
def test_lookup_codes_maps_crypto_majors_to_tradable_symbols() -> None:
    """CRYPTO_MAJORS must resolve to real codes, not the universe name itself.

    Regression: resolve_universe() accepts CRYPTO_MAJORS as a known universe,
    but _lookup_codes() had no crypto entries, so it fell through to
    [universe] and silently produced codes=["CRYPTO_MAJORS"] -- a literal
    string no loader can fetch.
    """
    assert _lookup_codes("CRYPTO_MAJORS") == ["BTC-USDT", "ETH-USDT"]
    assert _lookup_codes("crypto-majors") == ["BTC-USDT", "ETH-USDT"]
    assert "CRYPTO_MAJORS" not in _lookup_codes("CRYPTO_MAJORS")


def test_generate_backtest_config_crypto_majors_writes_tradable_codes(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """The full config-generation path must not emit the universe name as a code."""
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: tmp_path))
    hypothesis = _seed_hypothesis(tmp_path, monkeypatch, universe="CRYPTO_MAJORS")
    ...
    assert payload["config"]["codes"] == ["BTC-USDT", "ETH-USDT"]
    assert "CRYPTO_MAJORS" not in payload["config"]["codes"]


def test_generate_backtest_config_rejects_unsupported_universe(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Universes with no capable loader must still refuse, not substitute one."""
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: tmp_path))
    hypothesis = _seed_hypothesis(tmp_path, monkeypatch, universe="NOT_A_REAL_UNIVERSE")
