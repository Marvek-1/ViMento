...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("ETH/USDT", "ccxt"),
        ("local:my_file", "local"),
        ("something_weird", "akshare"),  # documented tokenless fallback (not tushare)
    ],
)
...
# --------------------------------------------------------------------------
# Structured verdict — _sources_used / _unresolved_reasons
#
# A caller (including an LLM reading the tool_result string) must never have
# to infer success/failure from a bare symbol list or a provider SDK's own
# stdout warning. Regression for a reproduced real failure: the same fetch
# call returned a bare {"_unresolved": [...]} on a transient network hiccup,
# with nothing distinguishing "provider temporarily unreachable" from "you
# need a Tushare token" — exactly the ambiguity a small model filled in
# with a fabricated explanation.
# --------------------------------------------------------------------------


def test_fetch_success_records_source_used() -> None:
    """A resolved symbol's actual provider is surfaced, not left implicit."""
    out = fetch_market_data(
        codes=["AAPL.US"],
        start_date="2026-01-01",
        end_date="2026-01-02",
    ...
    assert out["_sources_used"] == {"AAPL.US": "yahoo"}


def test_fetch_happy_path_omits_unresolved_reasons() -> None:
    """Additive-only: no _unresolved -> no _unresolved_reasons either."""
    out = fetch_market_data(
        codes=["AAPL.US"],
        start_date="2026-01-01",
        end_date="2026-01-02",
    ...
    assert "_unresolved_reasons" not in out
    assert "_unresolved" not in out


def test_fetch_loader_exception_reason_names_the_real_error() -> None:
    """A loader blow-up must surface the actual exception text, not a
    generic placeholder — this is the field an LLM should read instead of
    ...
    reason = out["_unresolved_reasons"]["X.US"]
    assert "loader exploded" in reason
    assert "yahoo" in reason


def test_fetch_missing_symbol_gets_generic_no_data_reason() -> None:
    """A loader that ran cleanly but omitted a symbol (no exception) still
    gets a reason string, distinct from an actual provider failure."""
    out = fetch_market_data(
        codes=["A.US", "B.US"],
        start_date="2026-01-01",
        end_date="2026-01-02",
    ...
    assert out["_unresolved_reasons"]["B.US"] == "loader returned no data for this symbol"
