...
class TestFallbackChains:
    ...
    def test_crypto_chain_includes_yfinance_fallback(self) -> None:
        """yfinance is a late-tier fallback for crypto when every exchange fails."""
        assert "yfinance" in FALLBACK_CHAINS["crypto"]
        # OKX (no credentials needed at all) leads; the named per-exchange
        # ccxt sources and the generic ccxt catch-all come next, all ahead
        # of yfinance/local.
        assert FALLBACK_CHAINS["crypto"][0] == "okx"
        assert FALLBACK_CHAINS["crypto"].index("yfinance") > FALLBACK_CHAINS["crypto"].index("ccxt")

    def test_crypto_chain_includes_named_exchange_sources(self) -> None:
        """Binance/Bybit/Gate.io are independently reachable, not hidden
        behind the generic ccxt source + CCXT_EXCHANGE env var."""
        chain = FALLBACK_CHAINS["crypto"]
        for name in ("binance", "bybit", "gate", "ccxt"):
            assert name in chain
        # None of the named exchanges is hardcoded as *the* default ahead of
        # the others — they sit together as a pool before the generic
        # catch-all and the non-exchange fallbacks.
        assert chain.index("okx") < chain.index("binance") < chain.index("ccxt")

    def test_named_exchange_sources_have_separate_modules(self) -> None:
        """Named CCXT exchanges should be visible as their own loader files."""
        from backtest.loaders.registry import LOADER_REGISTRY, _ensure_registered

        _ensure_registered()
        assert LOADER_REGISTRY["binance"].__module__ == "backtest.loaders.binance_loader"
        assert LOADER_REGISTRY["bybit"].__module__ == "backtest.loaders.bybit_loader"
        assert LOADER_REGISTRY["gate"].__module__ == "backtest.loaders.gate_loader"
    ...
    def test_unchanged_chains_preserved(self) -> None:
        """futures/fund/macro/forex chains must be left untouched (crypto's
        chain is deliberately changed — see test_crypto_chain_includes_named_exchange_sources)."""
...
# ---------------------------------------------------------------------------
# Real Tushare loader without a token — the fake-loader tests above prove the
# registry MECHANISM tolerates a raising __init__, but not that the actual
# production tushare.py loader integrates with it correctly. A future change
# to tushare.py's __init__/is_available, or a behavior change in the tushare
# PyPI package itself, would slip past every test above undetected.
# ---------------------------------------------------------------------------


class TestTushareNoTokenFallback:
    def test_auto_resolve_skips_tushare_without_token(self, monkeypatch) -> None:
        """resolve_loader('a_share') must never select tushare with no token set."""
        monkeypatch.delenv("TUSHARE_TOKEN", raising=False)
        loader = resolve_loader("a_share")
        assert loader.name != "tushare"

    def test_explicit_tushare_request_falls_back_without_token(self, monkeypatch) -> None:
        """An explicit source='tushare' request must degrade to a free source,
        not raise or surface the tushare SDK's own init error to the caller."""
        monkeypatch.delenv("TUSHARE_TOKEN", raising=False)
        cls = get_loader_cls_with_fallback("tushare")
        assert cls().name != "tushare"

    def test_tushare_becomes_available_once_token_is_set(self, monkeypatch) -> None:
        """Sanity check the guard is a real token check, not a stub that
        always reports unavailable regardless of configuration."""
        monkeypatch.setenv("TUSHARE_TOKEN", "a-real-looking-token")
        from backtest.loaders.tushare import DataLoader as TushareLoader

        assert TushareLoader().is_available() is True
