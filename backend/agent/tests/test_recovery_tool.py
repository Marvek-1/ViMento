...
class RecoveryToolTests(unittest.TestCase):
    def _load_recovery_module(self, *, trade_qty: float = 1.0):
        ...
        try:
            # agent/tests/test_recovery_tool.py -> parents[2] is the repo root,
            # where tools/recover_paper_sessions.py lives (sibling of agent/).
            path = Path(__file__).resolve().parents[2] / "tools" / "recover_paper_sessions.py"
