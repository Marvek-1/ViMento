...
def test_run_card_marks_scaffold_as_smoke_not_hypothesis_valid(tmp_path: Path) -> None:
    run_dir = tmp_path / "run"
    strategy_path = run_dir / "code" / "signal_engine.py"
    (run_dir / "code").mkdir(parents=True)
    (run_dir / "artifacts").mkdir(parents=True)
    (run_dir / "config.json").write_text('{"ok": true}\n', encoding="utf-8")
    (run_dir / "artifacts" / "metrics.csv").write_text("sharpe\n0\n", encoding="utf-8")
    (run_dir / "artifacts" / "equity.csv").write_text("date,equity\n", encoding="utf-8")
    strategy_path.write_text(
        '''
"""Auto-scaffolded signal engine for hypothesis hyp_test.
...
'''.lstrip(),
    ...
    assert card["strategy_implementation_status"] == "scaffold"
    assert card["provenance_valid"] is False
    assert card["has_trades"] is False
    assert card["statistically_evaluable"] is False
    assert card["hypothesis_supported"] is None
    assert card["run_purpose"] == "smoke_only"


def test_run_card_marks_implemented_strategy_with_artifacts_valid(
    tmp_path: Path,
) -> None:
    run_dir = tmp_path / "run"
    strategy_path = run_dir / "code" / "signal_engine.py"
    ...
    assert card["strategy_implementation_status"] == "implemented"
    assert card["provenance_valid"] is True
    assert card["has_trades"] is False
    assert card["statistically_evaluable"] is False
    assert card["hypothesis_supported"] is None
    assert card["run_purpose"] == "hypothesis_test"


def test_run_card_keeps_implemented_zero_trade_run_hypothesis_valid(
    tmp_path: Path,
) -> None:
    run_dir = tmp_path / "run"
    strategy_path = run_dir / "code" / "signal_engine.py"
    ...
    assert card["hypothesis_supported"] is None
    assert card["run_purpose"] == "hypothesis_test"
    assert card["metrics"]["trade_count"] == 0


def test_confidence_gate_min_trades_boundary(tmp_path: Path) -> None:
    """5, 199, and 200 trades exercise the statistical confidence gate boundary."""
    from backtest.run_card import CONFIDENCE_GATE_MIN_TRADES

    assert CONFIDENCE_GATE_MIN_TRADES == 200

    def _make_run(name: str, trade_count: int) -> dict:
        ...
        return write_run_card(
            ...
            {"trade_count": trade_count, "sharpe": 1.5, "total_return": 0.2},
            strategy_path=strategy_path,
        )

    five = _make_run("five", 5)
    assert five["has_trades"] is True
    assert five["statistically_evaluable"] is False
    assert five["hypothesis_supported"] is None

    below_gate = _make_run("below_gate", CONFIDENCE_GATE_MIN_TRADES - 1)
    assert below_gate["has_trades"] is True
    assert below_gate["statistically_evaluable"] is False
    assert below_gate["hypothesis_supported"] is None

    at_gate = _make_run("at_gate", CONFIDENCE_GATE_MIN_TRADES)
    assert at_gate["has_trades"] is True
    assert at_gate["window_integrity"] is True
    assert at_gate["statistically_evaluable"] is True
    assert at_gate["hypothesis_supported"] is None
